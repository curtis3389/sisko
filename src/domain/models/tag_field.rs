use super::{TagFieldType, TagId};
use crate::infrastructure::{multi_map, Entity, EntityId, MappingType};
...
pub struct TagFieldId {
    pub tag_id: TagId,
    pub tag_field_type: TagFieldType,
}

impl TagFieldId {
    pub fn new(tag_id: TagId, tag_field_type: TagFieldType) -> Self {
        Self {
            tag_id,
            tag_field_type,
        }
...
impl EntityId for TagFieldId {
    fn to_string(&self) -> String {
        String::from(self)
    }
}

impl From<&TagFieldId> for String {
    fn from(id: &TagFieldId) -> Self {
        format!("{}:{}", id.tag_id.to_string(), id.tag_field_type)
    }
}
...
pub enum TagField {
    ...
    Binary(TagFieldId, Vec<u8>, Option<Vec<u8>>),

    /// A field containing text data.
    Text(TagFieldId, String, Option<String>),

    /// An unrecognized field.
    Unknown(TagFieldId, String),
}

impl TagField {
    fn new_for_frame(tag_id: &TagId, frame: &ID3v2Frame) -> Self {
        match &frame.fields {
            ID3v2FrameFields::TextFields { encoding: _, text } => TagField::Text(
                TagFieldId {
                    tag_id: tag_id.clone(),
                    tag_field_type: TagFieldType::from(frame),
                },
                text[0].clone(),
                None,
            ),
    ...
    pub fn parse_all(tag_id: &TagId, frames: &[ID3v2Frame]) -> Vec<TagField> {
        #[derive(Clone, Debug)]
        enum MultiFrameType {
            Ipls,
            Tpos,
        ...
        multi_map(
            frames,
            |a| Ok(TagField::new_for_frame(tag_id, a)),
            |g, v| -> Result<TagField> {
                match g {
                    PartialFrameGroup::Date => TagField::date(tag_id, v),
                }
            },
            |t, frame| -> Result<Vec<TagField>> {
                Ok(match t {
                    MultiFrameType::Ipls => TagField::ipls(tag_id, frame),
                    MultiFrameType::Tpos => TagField::tpos(tag_id, frame),
                    MultiFrameType::Trck => TagField::trck(tag_id, frame),
                })
            },
        )
        .unwrap()
    ...
    pub fn date(tag_id: &TagId, frames: Vec<&ID3v2Frame>) -> Result<TagField> {
        let tyer = frames
            .iter()
        ...
        Ok(TagField::Text(
            TagFieldId {
                tag_id: tag_id.clone(),
                tag_field_type: TagFieldType::Date,
            },
            s,
            None,
        ))
    ...
    pub fn ipls(tag_id: &TagId, frame: &ID3v2Frame) -> Vec<TagField> {
        match &frame.fields {
            ID3v2FrameFields::TextFields { encoding: _, text } => text
                ...
                .map(|(a, s)| {
                    TagField::Text(
                        TagFieldId {
                            tag_id: tag_id.clone(),
                            tag_field_type: match a.trim().to_lowercase().as_str() {
    ...
    pub fn tpos(tag_id: &TagId, frame: &ID3v2Frame) -> Vec<TagField> {
        match &frame.fields {
    ...
    pub fn trck(tag_id: &TagId, frame: &ID3v2Frame) -> Vec<TagField> {
        match &frame.fields {
            ID3v2FrameFields::TextFields { encoding: _, text } => text[0]
                .split('/')
                .map(String::from)
    ...
    pub fn tag_field_type(&self) -> TagFieldType {
        match &self {
            TagField::Binary(id, _, _) => id.tag_field_type.clone(),
            TagField::Text(id, _, _) => id.tag_field_type.clone(),
            TagField::Unknown(id, _) => id.tag_field_type.clone(),
        }
    }
}

impl Entity for TagField {
    type Id = TagFieldId;
    ...
    {
        match self {
            TagField::Binary(id, _, _) => id,
            TagField::Text(id, _, _) => id,
            TagField::Unknown(id, _) => id,
        }
    }
}
